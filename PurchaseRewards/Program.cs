        ...
        static void Main(string[] args)
        {
            ...
            if (transactions != null)
            {
                foreach (string transactionLine in transactions)
                {
                    ...
                    transactionProcessor.processTransaction(transactionLine, transactionList);
                }
            }
            if (transactionList.Count > 0)
            {
                ...
                List<Transaction> threeMonthsOfTransactions = transactionProcessor.processThreeMonthsOfTransactions(transactionList, startDate);
                    ...
                    if (transactionListCustomerGroups != null)
                    {
                        foreach (var group in transactionListCustomerGroups)
                        {
                            ...
                            foreach (var month in months)
                            {
                                ...
                                Console.WriteLine("For Customer " + group.Key + " and month " + month.Select(x => x.TransactionDate.Month).First() + " total points = " + totalPoints);
                            }
                        }
